    ...
    if (already === null) {
      ...
      if (selected2) {
        await document
          .collection("choice2Users")
          .add({ user: userObj.displayName });
        await document.collection("choice1Users").doc(alreadyId).delete();
        setAlready("selected2");
      } else if (!selected1 && !selected2) {
        await document.collection("choice1Users").doc(alreadyId).delete();
        setAlready(null);
      ...
      if (selected1) {
        await document
          .collection("choice1Users")
          .add({ user: userObj.displayName });
        await document.collection("choice2Users").doc(alreadyId).delete();
        setAlready("selected1");
      } else if (!selected1 && !selected2) {
        await document.collection("choice2Users").doc(alreadyId).delete();
        setAlready(null);
    ...
    <>
      {init && (
        <article className="ChoiceInfo">
          ...
          <div className="ChoiceInfo-writer">{item.writer} 님의 고민!</div>
            ...
            <div className="ChoiceInfo-choice1">
              ...
              <h4 className="ChoiceInfo-percentage">
                {choice1Users === 0 && choice2Users === 0
                  ? 0
                  : Math.floor(
                      (100 * choice1Users) / (choice1Users + choice2Users)
                    )}
          ...
          <div className="ChoiceInfo-fixedBtns">
            ...
            {userObj.displayName === item.writer && (
              <button onClick={toggleModal} className="ChoiceInfo-deleteBtn">
                DELETE
              </button>
            )}
          </div>
